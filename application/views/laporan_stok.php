				...
				<?php
					...
					foreach($data as $d){
						if($d['serial'] == 1){
							...
							if(!empty($url)) $this->db->where('stok_keluar.id_cabang',$url);
							$this->db->where('id_item',$d['id']);
							...
							if(!empty($url)) $this->db->where('pembelian.id_cabang',$url);
							$this->db->where('id_item',$d['id']);
							...
							$total = ($pembelian_unit + $to_unit) - ($from_unit + $transaksi_unit) - ($return_unit + $stok_keluar_unit + $service_unit);
						}
						if($total != 0){
							$d['total'] = $total;
							$item[] = $d;
